...
export const SOURCE_CONNECTOR_TYPE_ZENPROXY_CLIENT = "zenproxy_client";
...
const BARE_PROXY_INPUT_REGEX =
  /^(?<auth>[^@\s]+@)?(?<host>\[[^\]]+\]|[^:\s/]+):(?<port>\d{1,5})$/i;
...
export function normalizeDirectProxyInput(
  value,
  defaultScheme = DEFAULT_DIRECT_PROXY_SCHEME,
) {
  const trimmed = normalizeString(value);
  if (!trimmed) return "";
...
export function inferSourceKind(value, explicitKind = "") {
  ...
  if (normalizedKind === SOURCE_KIND_SUBSCRIPTION)
    return SOURCE_KIND_SUBSCRIPTION;
...
function mergeSourceOptions(item, kind) {
  const existingOptions =
    item && typeof item.options === "object" && item.options !== null
      ? { ...item.options }
      : {};

  if (kind === SOURCE_KIND_SUBSCRIPTION) {
    if (normalizeString(item?.exclude)) existingOptions.exclude = item.exclude;
    if (normalizeString(item?.customUserAgent))
      existingOptions.customUserAgent = item.customUserAgent;
    if (normalizeString(item?.fetchProxy))
      existingOptions.fetchProxy = item.fetchProxy;
    if (item?.plusAsSpace === true) existingOptions.plusAsSpace = true;
  }

  if (kind === SOURCE_KIND_CONNECTOR) {
    if (normalizeString(item?.connector_type))
      existingOptions.connector_type = item.connector_type;
...
export function normalizeSourceItem(item, options = {}) {
  ...
  const defaultDirectProxyScheme =
    normalizeString(options.defaultDirectProxyScheme) ||
    DEFAULT_DIRECT_PROXY_SCHEME;
  const rawInput = normalizeString(source.input || source.url);
  const normalizedInput = normalizeDirectProxyInput(
    rawInput,
    defaultDirectProxyScheme,
  );
  ...
  if (kind === SOURCE_KIND_CONNECTOR) {
    ...
    normalized.connector_config =
      ...
      typeof normalized.options.connector_config === "object"
        ? { ...normalized.options.connector_config }
        : {};
  } else {
...
export function isEchWorkerSource(item) {
  ...
  return (
    normalized.kind === SOURCE_KIND_CONNECTOR &&
    normalized.connector_type === SOURCE_CONNECTOR_TYPE_ECH_WORKER
  );
}
...
export function getSourceKey(item) {
  ...
  if (normalized.kind === SOURCE_KIND_CONNECTOR) {
    const connectorOptions = {
      ...
      connector_config:
        normalized.connector_config &&
        typeof normalized.connector_config === "object"
...
export function shouldProbeSource(item) {
  ...
  if (
    ![SOURCE_KIND_SUBSCRIPTION, SOURCE_KIND_PROXY_URI].includes(normalized.kind)
  ) {
    return false;
  }
...
export function canManuallyProbeSource(item) {
  ...
  if (!normalized.input) return false;
  return (
    shouldProbeSource(normalized) || isLikelyHTTPProxyInput(normalized.input)
  );
}
...
export function isSourceProbeMismatch(item) {
  const normalized = normalizeSourceItem(item);
  if (
    !normalized.detected_kind ||
    normalized.detected_kind === SOURCE_DETECTED_KIND_UNKNOWN
  ) {
    return false;
  }
...
export function getSourceProbeSummary(item) {
  ...
  if (normalized.probe_status === SOURCE_PROBE_STATUS_SKIPPED) {
    return {
      ...
      label:
        normalized.detected_kind === SOURCE_KIND_PROXY_URI
          ? "结构像代理"
          : "结构已判定",
      tone: "muted",
      mismatch: isSourceProbeMismatch(normalized),
      description:
        normalized.probe_message || "输入结构已经足够明确，未执行联网探测。",
    };
  }
  ...
  if (normalized.probe_status === SOURCE_PROBE_STATUS_VERIFIED) {
    ...
    return {
      ...
      mismatch: false,
      description:
        normalized.probe_message || "联网探测结果与当前声明类型一致。",
    };
  }
  ...
  if (normalized.probe_status === SOURCE_PROBE_STATUS_UNREACHABLE) {
    return {
      ...
      mismatch: false,
      description:
        normalized.probe_message || "本次没有成功拉取到可判断的内容。",
    };
  }
  ...
  return {
    ...
    mismatch: false,
    description:
      "保存后会对歧义 http(s) 来源做一次辅助探测，但不会自动改写类型。",
  };
}
