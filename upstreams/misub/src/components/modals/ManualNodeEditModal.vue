...
const { handleBulkImport } = useBulkImportLogic({
  addSubscriptionsFromBulk,
  addNodesFromBulk,
});

const defaultConnectorConfigFor = (type) => {
  if (type === SOURCE_CONNECTOR_TYPE_ZENPROXY_CLIENT) {
    return {
      count: 10,
      fetch_path: "/api/client/fetch",
      country: "",
      type: "",
      proxy_id: "",
      api_key: "",
...
const applyConnectorConfigDefaults = (type) => {
  if (!props.editingNode) return;
  props.editingNode.connector_config = {
    ...defaultConnectorConfigFor(type),
    ...(props.editingNode.connector_config || {}),
  };
};
...
const sourceKindModel = computed({
  get: () =>
    normalizeSourceItem(props.editingNode).kind || SOURCE_KIND_PROXY_URI,
  set: (value) => {
    if (!props.editingNode) return;
    props.editingNode.kind = value;
    if (value === SOURCE_KIND_CONNECTOR) {
      props.editingNode.connector_type =
        props.editingNode.connector_type || SOURCE_CONNECTOR_TYPE_ECH_WORKER;
      applyConnectorConfigDefaults(props.editingNode.connector_type);
    }
  },
});

const isConnectorMode = computed(
  () => sourceKindModel.value === SOURCE_KIND_CONNECTOR,
);
...
const connectorTypeModel = computed({
  ...
  set: (value) => {
    ...
    applyConnectorConfigDefaults(value);
  },
});

const isECHWorkerMode = computed(
  () => connectorTypeModel.value === SOURCE_CONNECTOR_TYPE_ECH_WORKER,
);
const isZenProxyMode = computed(
  () => connectorTypeModel.value === SOURCE_CONNECTOR_TYPE_ZENPROXY_CLIENT,
);
const connectorTypeLabel = computed(() =>
  isZenProxyMode.value ? "ZenProxy Client" : "ECH Worker",
);
...
const connectorConfig = computed(() => {
  ...
  if (
    !props.editingNode.connector_config ||
    typeof props.editingNode.connector_config !== "object"
  ) {
    props.editingNode.connector_config = {};
  }
...
const getProtocol = (url) => {
  ...
  if (isConnectorMode.value) {
    return isZenProxyMode.value
      ? { name: "ZEN", color: "orange" }
      : { name: "ECH", color: "cyan" };
  }
  const lowerUrl = url.toLowerCase().trim();
  if (lowerUrl.startsWith("anytls://"))
    return { name: "AnyTLS", color: "slate" };
  ...
  if (lowerUrl.startsWith("snell://"))
    return { name: "Snell", color: "indigo" };
...
const extractNodeName = (url) => {
  ...
  try {
    if (isConnectorMode.value) {
      ...
      return `${isZenProxyMode.value ? "ZenProxy" : "ECH"} ${parsed.hostname}`;
    }
    // VMess 特殊处理
...
const reprobeButtonTitle = computed(() => {
  if (isConnectorMode.value)
    return `${connectorTypeLabel.value} connector 不参与辅助探测`;
    ...
    <template #title>
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p
            v-else-if="isConnectorMode"
            class="text-sm text-cyan-600 dark:text-cyan-300 mt-0.5 font-medium"
          >
            当前模式：{{ connectorTypeLabel }}
          </p>
        </div>
      </div>
    </template>
      ...
      <div class="space-y-5">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <button
            ...
            :class="
              sourceKindModel === SOURCE_KIND_PROXY_URI
          ...
          >
            ...
            <div class="text-xs opacity-80 mt-1">
              支持手填 `vmess://`、`socks5://`、`http://user:pass@host:port`
              等输入，也支持多行批量导入。
            </div>
          </button>
          <button
            ...
            class="text-left px-4 py-3 misub-radius-lg border transition-colors"
            :class="
              sourceKindModel === SOURCE_KIND_CONNECTOR
                ? 'border-cyan-500 bg-cyan-50/80 text-cyan-700 dark:border-cyan-400 dark:bg-cyan-500/10 dark:text-cyan-200'
                : 'border-gray-200 bg-white/70 text-gray-700 dark:border-white/10 dark:bg-white/5 dark:text-gray-300'
            "
          >
            <div class="font-semibold">Connector Source</div>
            <div class="text-xs opacity-80 mt-1">
              作为 `connector` source 保存到 MiSub，后续可被 profile 和 manifest
              选择。当前支持 ECH Worker 与 ZenProxy Client。
            </div>
          </button>
        </div>
        ...
        >
          <div class="flex flex-col">
            ...
            >
              <option :value="SOURCE_CONNECTOR_TYPE_ECH_WORKER">
                ECH Worker
              </option>
              <option :value="SOURCE_CONNECTOR_TYPE_ZENPROXY_CLIENT">
                ZenProxy Client
              </option>
            </select>
          </div>

          <div v-if="isECHWorkerMode" class="flex flex-col">
            <label
              class="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1.5 ml-1"
            >
              本地下游协议
          ...
          <template v-if="isZenProxyMode">
            <div class="flex flex-col">
              ...
              <input
                v-model="connectorConfig.api_key"
                type="text"
                placeholder="ZenProxy user API key"
                class="block w-full px-3 py-2.5 bg-white/70 dark:bg-gray-900/50 border border-gray-200/80 dark:border-white/10 misub-radius-lg shadow-sm focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-500 sm:text-sm dark:text-white transition-colors"
              />
            </div>
            ...
            <div class="flex flex-col">
              ...
              <input
                v-model.number="connectorConfig.count"
                type="number"
                min="1"
                step="1"
                placeholder="10"
            ...
            <div class="flex flex-col">
              ...
              <input
                v-model="connectorConfig.country"
                type="text"
            ...
            <div class="flex flex-col">
              ...
              <input
                v-model="connectorConfig.type"
                type="text"
                placeholder="vmess / vless / trojan / shadowsocks"
                class="block w-full px-3 py-2.5 bg-white/70 dark:bg-gray-900/50 border border-gray-200/80 dark:border-white/10 misub-radius-lg shadow-sm focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-500 sm:text-sm dark:text-white transition-colors"
              />
            </div>
            ...
            <div class="flex flex-col">
              ...
              <input
                v-model="connectorConfig.proxy_id"
                type="text"
                placeholder="固定某个 ZenProxy 代理 ID"
                class="block w-full px-3 py-2.5 bg-white/70 dark:bg-gray-900/50 border border-gray-200/80 dark:border-white/10 misub-radius-lg shadow-sm focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-500 sm:text-sm dark:text-white transition-colors"
              />
            </div>
            ...
            <div class="flex flex-col">
              ...
              <input
                v-model="connectorConfig.risk_max"
                type="number"
                min="0"
                max="1"
                step="0.01"
            ...
            <div class="flex flex-col">
              ...
              <input
                v-model="connectorConfig.fetch_path"
                type="text"
                placeholder="/api/client/fetch"
                class="block w-full px-3 py-2.5 bg-white/70 dark:bg-gray-900/50 border border-gray-200/80 dark:border-white/10 misub-radius-lg shadow-sm focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-500 sm:text-sm dark:text-white transition-colors"
              />
            </div>
          </template>
        ...
        >
          <label
            class="flex items-center gap-2 px-3 py-2 misub-radius-lg border border-gray-200/80 dark:border-white/10 bg-white/60 dark:bg-white/5 text-sm text-gray-700 dark:text-gray-300"
          >
            <input
              v-model="connectorConfig.chatgpt"
              type="checkbox"
          ...
          >
            <input
              v-model="connectorConfig.google"
              type="checkbox"
              class="rounded border-gray-300 text-cyan-600 focus:ring-cyan-500"
            />
            Google 可用
          ...
          >
            <input
              v-model="connectorConfig.residential"
              type="checkbox"
              class="rounded border-gray-300 text-cyan-600 focus:ring-cyan-500"
            />
            Residential
            ...
            <div class="flex h-full">
              ...
              <textarea
                ...
                :placeholder="
                  isConnectorMode
                    ? isZenProxyMode
                      ? '输入 ZenProxy 服务地址，例如 https://zenproxy.top'
        ...
        <div
          v-if="isConnectorMode"
          class="misub-radius-lg border px-4 py-3 text-sm bg-cyan-50/70 text-cyan-700 border-cyan-200 dark:bg-cyan-500/10 dark:text-cyan-200 dark:border-cyan-500/20"
        >
          <div class="font-semibold mb-1">{{ connectorTypeLabel }} 说明</div>
          <div v-if="isECHWorkerMode">
            这类来源会以 `connector` source 的形式进入 MiSub，可以被 profile
            选择并出现在 machine manifest
            中，但不会被当前页面当成普通代理节点测速。
          </div>
          <div v-else>
            这类来源会在 EasyProxy 侧按 `connector` 执行：运行时请求 ZenProxy
            `/api/client/fetch`，把返回的 outbound 动态 materialize
            为临时上游节点。
          </div>
        </div>
        ...
        <div
          class="misub-radius-lg border px-4 py-3 text-sm"
          :class="{
            'bg-amber-50/80 text-amber-700 border-amber-200 dark:bg-amber-500/10 dark:text-amber-200 dark:border-amber-500/20':
              probeSummary.tone === 'warning',
            'bg-red-50/80 text-red-700 border-red-200 dark:bg-red-500/10 dark:text-red-200 dark:border-red-500/20':
              probeSummary.tone === 'danger',
            'bg-emerald-50/80 text-emerald-700 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-200 dark:border-emerald-500/20':
              probeSummary.tone === 'success',
            'bg-gray-50/80 text-gray-600 border-gray-200 dark:bg-white/5 dark:text-gray-300 dark:border-white/10':
              !['warning', 'danger', 'success'].includes(probeSummary.tone),
          }"
        >
          <div class="font-semibold mb-1">{{ probeSummary.label }}</div>
          <div>{{ probeSummary.description }}</div>
          <div
            v-if="detectedKindText || lastProbeAtText"
            class="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs opacity-80"
          >
