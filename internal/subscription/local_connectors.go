...
func (m *Manager) RefreshRuntimeSources(ctx context.Context) error {
	...
	cfg := m.baseCfg
	boxMgr := m.boxMgr
	hasSources := cfg != nil && (len(cfg.Subscriptions) > 0 ||
		hasEnabledLocalConnectors(cfg.Connectors) ||
		(cfg.SourceSync.Enabled && (strings.TrimSpace(cfg.SourceSync.ManifestURL) != "" || len(cfg.SourceSync.FallbackSubscriptions) > 0)))
	m.mu.RUnlock()
